    ...
    return (
        <div className="mt-3">
            ...
            {reviews && reviews.length > 0 ? (
                    ...
                    <div key={review.DanhGiaID} className="review-card">
                        <div className="review-header">
                            ...
                            <div className="review-date">
                                {new Date(review.NgayCapNhat || review.NgayTao).toLocaleDateString("vi-VN")}
                                {review.NgayCapNhat && <span className="fst-italic"> (đã chỉnh sửa)</span>}
                            </div>
                        </div>
                        
                        {review.ThuocTinh && (
                            <p className="text-muted small mb-2 fst-italic">Phân loại: {review.ThuocTinh}</p>
                        )}
                        ...
                        <div className="review-media-grid">
                            {review.HinhAnhURL && (
                                <div className="review-media-thumbnail" onClick={() => handleShowImageModal(review.HinhAnhURL)}>
                                    <Image src={review.HinhAnhURL} thumbnail />
                                </div>
                            )}
                            {review.VideoURL && (
                                <div className="review-media-thumbnail video-thumbnail" onClick={() => handleShowVideoModal(review.VideoURL)}>
                ...
                <p className="text-center text-muted py-4">Hiện chưa có đánh giá nào.</p>
            )}
            <Alert variant="info" className="mt-4">
                Để lại đánh giá? Vui lòng mua hàng và đánh giá từ trang <Link to="/profile/orders">Đơn hàng của tôi</Link>.
            </Alert>
